...
class OtpSendResponse(BaseModel):
    ...
    phone: str
    message: str
    # dev_code et debug_code contiennent le même code en mode démo.
    # (debug_code est le nom attendu par le frontend codena.sn.)
...
class OtpVerifyRequest(BaseModel):
    phone: str
    # Le code peut arriver sous "code" (défaut) ou "otp" selon le client.
    code: str | None = None
    otp: str | None = None
...
# Les deux styles de chemin sont acceptés : "/auth/otp-send" et "/auth/otp/send"
# (ce dernier est celui utilisé par le frontend codena.sn).
...
def otp_send(req: OtpSendRequest):
    ...
    exposed = code if EXPOSE_OTP else None
...
def otp_verify(req: OtpVerifyRequest):
    ...
    if req.resolved_code() != entry["code"]:
